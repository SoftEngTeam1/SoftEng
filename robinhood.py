...
def robin_login():
    ...
    try:
        login = robin.login(USERNAME, PASS, store_session=False)
...
def get_watchlist_symbols(): 
    ...
    #Can iterate through all watchlists, but this is set for a specific WatchList
    for name in watchlist_names:
        if name == watchlist: 
            ...
            for item in lists:
                ...
                # instrument_data = robin.get_instrument_by_url(item.get('instrument'))
...
def get_position_creation_date(symbol, holdings_data):
    ...
    Returns:
        String containing the date and time the stock was bought, or "Not found" 
    """
...
def search_stock(ticker): 
    ...
    stock_data = robin.get_stock_historicals(ticker,span='week',bounds='regular')
